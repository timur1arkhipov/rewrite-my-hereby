...
const terebyfileRegExp = /^terebyfile\.m?js$/i;

export function findTerebyfile(dir: string): string {
    const result = findUp(dir, (dir) => {
        const entries = fs.readdirSync(dir, { withFileTypes: true });
        const matching = entries.filter((e) => terebyfileRegExp.test(e.name));
        if (matching.length > 1) {
            throw new UserError(`Found more than one Terebyfile: ${matching.map((e) => e.name).join(", ")}`);
        }
        if (matching.length === 1) {
    ...
    throw new UserError("Unable to find Terebyfile.");
}

export interface Terebyfile {
    readonly tasks: ReadonlyMap<string, Task>;
    readonly defaultTask: Task | undefined;
}

export async function loadTerebyfile(terebyfilePath: string): Promise<Terebyfile> {
    // Note: calling pathToFileURL is required on Windows to disambiguate URLs
    // from drive letters.
    const terebyfile = await import(pathToFileURL(terebyfilePath).toString());

    const exportedTasks = new Set<Task>();
    let defaultTask: Task | undefined;

    for (const [key, value] of Object.entries(terebyfile)) {
